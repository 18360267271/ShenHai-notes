...
```
export DAVINCI3_HOME=/opt/module/davinciCopy to clipboardErrorCopied
```
...
```
mysql -P 3306 -h localhost -u root -proot davinci0.3 < $DAVINCI3_HOME/bin/davinci.sqlCopy to clipboardErrorCopied
```

运行脚本初始化数据库（注：由于 Davinci 系统数据库中包含存储过程，请务必在创建数据库时赋予执行权限）

```
sh bin/initdb.shCopy to clipboardErrorCopied
```
...
![image-20210604111545332](https://glong1997.github.io/XiYun-Notes/images/image-20210604111545332.png)

在bin目录下启动关闭服务

```
./start-server.shCopy to clipboardErrorCopied
```
...
>
> 解决方法：[https://blog.csdn.net/qq_38455201/article/details/80776446评论区](https://blog.csdn.net/qq_38455201/article/details/80776446%E8%AF%84%E8%AE%BA%E5%8C%BA)
>
...
```
git clone https://github.com/edp963/davinci-dockerCopy to clipboardErrorCopied
version: '3.6'
services:
  davinci:
    environment:
